...
int main(){
    int p;
    int *burst, *arrival, *priority, *finish, *flag, *wait, *turnaround,*position;
        ...
        for(int l=0;l<p;l++){
            if(flag[l]==1 && finish[l]==0){
                ...
                if(temp<temp1){
                    ...
                    for(int x=0;x<p;x++){
                        if(flag[x]==1 && finish[x]==0){
                            if(position[x]<position[l]){
                                temp_address=x;
                                temp1 =temp;
                            }
                        }
    ...
    for(int i=0;i<p;i++){
        cout<<" P"<<i+1<<" -             "<<finish[i]<<"               "<<wait[i]<<"              "<<turnaround[i]<<endl;
    }
    float avg_wait=0;
    float avg_turn=0;
    for(int i=0;i<p;i++){
